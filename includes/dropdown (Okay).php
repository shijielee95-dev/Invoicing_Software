...
/**
 * renderSearchableDropdown2()
 * Same proven Alpine pattern as renderDropdown(), but with a text input
 * so the user can type to filter. @blur closes after 200ms, @mousedown.prevent picks.
 */
function renderSearchableDropdown2(
    string $name,
    array  $options,
    string $selected    = '',
    string $placeholder = 'Select or type...',
    ...
    $alpineOptions = [];
    foreach ($options as $val => $label) {
        $alpineOptions[] = ['value' => (string)$val, 'text' => (string)$label];
    }
    ...
    $ec   = $extraClass ? ' ' . $extraClass : '';
    $ph   = htmlspecialchars($placeholder, ENT_QUOTES);
    $nm   = htmlspecialchars($name, ENT_QUOTES);
    ...
    if (strpos($extraClass, 'w-full') !== false) {
        ...
        $longest = $placeholder;
        foreach ($options as $label) {
            if (mb_strlen((string)$label) > mb_strlen($longest)) $longest = (string)$label;
        }
        $widthStyle = 'style="width:' . (mb_strlen($longest) * 0.6 + 2) . 'rem"';
    }

    $filter = 'options.filter(function(o){return !q||o.text.toLowerCase().indexOf(q.toLowerCase())!==-1;})';
    $find   = 'options.find(function(o){return o.value===value;})';
    ?>
    <div x-data='<?= $state ?>' x-init="_esc=false" class="relative<?= $ec ?>" <?= $widthStyle ?>>

        <div class="relative">
            <input type="text" x-ref="inp"
                :value="open ? q : (<?= $find ?> ? <?= $find ?>.text : '')"
                @focus="if(_esc){return;} open=true; q=''"
                @input="q=$event.target.value; open=true"
                @blur="setTimeout(function(){ open=false; q=''; _esc=false; }, 200)"
                @keydown.escape="
                    open=false; q=''; _esc=true;
                    var _t=<?= $find ?>; var _txt=_t?_t.text:'';
                    if($refs.inp){$refs.inp.blur(); setTimeout(function(){if($refs.inp)$refs.inp.value=_txt;},0);}
                "
                placeholder="<?= $ph ?>"
                autocomplete="off"
                class="w-full h-9 px-3 pr-8 rounded-lg bg-white border border-slate-300 text-sm text-black focus:outline-none focus:border-indigo-500 transition hover:border-slate-400">
        ...
        <div x-show="open"
            ...
            <ul class="max-h-56 overflow-y-auto py-1">
                <template x-for="item in <?= $filter ?>" :key="item.value">
                    <li>
                        <button type="button"
                            @mousedown.prevent="value=item.value; open=false; q=''; $nextTick(function(){ if($refs.inp){$refs.inp.value=item.text;$refs.inp.blur();} })"
                            class="w-full text-left px-3 py-1.5 text-sm transition-colors"
                ...
                <li x-show="<?= $filter ?>.length===0" class="px-3 py-2 text-sm text-slate-400 italic">No results</li>
            </ul>
        </div>
        ...
        <select name="<?= $nm ?>"
            ...
            <?php foreach ($options as $val => $label):
                $sel = ($selected === (string)$val) ? ' selected' : ''; ?>
            <option value="<?= htmlspecialchars((string)$val, ENT_QUOTES) ?>"<?= $sel ?>><?= htmlspecialchars((string)$label, ENT_QUOTES) ?></option>
            <?php endforeach; ?>
        </select>
    </div>
    <?php
