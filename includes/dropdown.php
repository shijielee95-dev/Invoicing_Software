...
x-data="{
    ...
    get filtered() {
        return this.options.filter(o =>
            !this.q || o.text.toLowerCase().includes(this.q.toLowerCase())
        );
    },

    get selectedText() {
        let f = this.options.find(o => o.value === this.value);
        return f ? f.text : '';
    },

    initHighlight() {
        let f = this.filtered;
        let index = f.findIndex(o => o.value === this.value);
        this._ai = index >= 0 ? index : 0;
    },
    ...
    closeDropdown() {
        ...
        this.$nextTick(() => {
            if (this.$refs.inp) {
                this.$refs.inp.value = this.selectedText;
                this.$refs.inp.blur();
            }
        });
    }
...
>
    ...
    <div class="relative">
        <input type="text"
            ...
            @input="q = $event.target.value; open = true; _ai = 0"
            ...
            @keydown="
                ...
                switch ($event.key) {
                    ...
                    case 'ArrowDown':
                        ...
                        _ai = (_ai < _f.length - 1) ? _ai + 1 : 0;
                        break;

                    case 'ArrowUp':
                        $event.preventDefault();
                        if (_f.length === 0) return;
                        _ai = (_ai > 0) ? _ai - 1 : _f.length - 1;
                        break;

                    case 'Enter':
                        $event.preventDefault();
                        if (_f.length === 0) return;
                        let picked = (_ai >= 0) ? _f[_ai] : _f[0];
                        value = picked.value;
                        closeDropdown();
                        break;
                ...
                $nextTick(() => {
                    if ($refs.ul) {
                        let btns = $refs.ul.querySelectorAll('button');
                        if (btns[_ai]) btns[_ai].scrollIntoView({ block: 'nearest' });
                    }
                });
            "
    ...
    <div x-show="open"
        style="display:none"
        class="absolute z-[9996] mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden"
    >
        <ul class="max-h-56 overflow-y-auto py-1" x-ref="ul">
    ...
    <select name="<?= $nm ?>"
        ...
        <?php foreach ($options as $val => $label): ?>
            <option value="<?= htmlspecialchars((string)$val, ENT_QUOTES) ?>"
                <?= ($selected === (string)$val) ? 'selected' : '' ?>>
                <?= htmlspecialchars((string)$label, ENT_QUOTES) ?>
            </option>
        <?php endforeach; ?>
