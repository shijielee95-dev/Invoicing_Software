...
function computeNextNo(PDO $pdo, string $format): string {
    ...
    $pdo->prepare("INSERT IGNORE INTO quotation_sequences (prefix, year, next_no) VALUES (?, ?, 1)")
        ->execute([$seqKey, $year]);
    $stmt = $pdo->prepare("SELECT next_no FROM quotation_sequences WHERE prefix=? AND year=?");
    $stmt->execute([$seqKey, $year]);
    $seq = (int)$stmt->fetchColumn();

    // Walk forward past any numbers already used
    $check = $pdo->prepare("SELECT COUNT(*) FROM quotations WHERE quotation_no=?");
...
$stats = $pdo->query("
    SELECT
        ...
        COALESCE(SUM(CASE WHEN status='accepted' THEN total_amount END), 0) AS revenue
    FROM quotations
")->fetch();
...
if ($search !== '') {
    $where[]  = '(i.quotation_no LIKE ? OR i.customer_name LIKE ?)';
    $params[] = "%$search%";
    $params[] = "%$search%";
}
if ($statusF !== '') {
...
if ($dateFrom !== '') { $where[] = 'DATE(i.quotation_date) >= ?'; $params[] = $dateFrom; }
if ($dateTo   !== '') { $where[] = 'DATE(i.quotation_date) <= ?'; $params[] = $dateTo; }

$sql = "
    SELECT i.id, i.quotation_no, i.customer_name, i.quotation_date, i.due_date,
...
$quotationsForJs = array_map(function($inv) use ($statusBadgeMap) {
    $overdue = false; // quotations do not go overdue
    return [
        'id'            => (int)$inv['id'],
        'quotation_no'    => $inv['quotation_no'],
        'customer'      => $inv['customer_name'],
        'quotation_date'  => $inv['quotation_date'] ? date('d M Y', strtotime($inv['quotation_date'])) : '',
...
$quotationsJson = json_encode(array_values($quotationsForJs), JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT);

layoutOpen('Quotations', number_format($stats['total']) . ' total quotations');
?>
<style>
...
document.getElementById('pageActions').innerHTML =
    '<a href="quotation.php?action=new" class="<?= t('btn_base').' '.t('btn_primary') ?> h-9">' +
...
<div class="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-5">
    ...
    $cards = [
        ...
        ['label'=>'Accepted', 'value'=>$stats['accepted'], 'sub'=>rm($stats['revenue']), 'filter'=>'accepted'],
        ['label'=>'Sent',     'value'=>$stats['sent'],     'sub'=>'Awaiting response',   'filter'=>'sent'],
        ['label'=>'Draft',    'value'=>$stats['draft'],    'sub'=>'Not yet sent',        'filter'=>'draft'],
        ['label'=>'Rejected', 'value'=>$stats['rejected'], 'sub'=>'Not accepted',        'filter'=>'rejected'],
    ];
    foreach ($cards as $c):
...
<form id="filterForm" method="GET" class="<?= t('card') ?> flex flex-wrap items-end gap-3 mb-5"
    ...
    <div>
        ...
        <?php renderDropdown('status', [''  => 'All Status', 'draft' => 'Draft', 'sent' => 'Sent', 'accepted' => 'Accepted', 'rejected' => 'Rejected', 'cancelled' => 'Cancelled'], $statusF); ?>
    </div>

    <div>
        <?php /* No LHDN filter for quotations */ ?>
    </div>

    <div class="flex gap-2">
        <button type="submit" class="<?= t('btn_base') ?> <?= t('btn_primary') ?> h-9">Filter</button>
        <?php if ($search || $statusF || $lhdnF || $dateFrom || $dateTo): ?>
        <a href="quotation.php" class="<?= t('btn_base') ?> <?= t('btn_ghost') ?> h-9">Clear</a>
        <?php endif; ?>
    </div>
                ...
                <table class="text-sm" style="table-layout:fixed;width:100%">
                    <thead id="invThead">
                        <tr class="flex w-full">
                            <th class="<?= t('th') ?>" style="flex:14 1 0;min-width:90px" data-col="quotation_no">Quotation #</th>
                            <th class="<?= t('th') ?>" style="flex:20 1 0;min-width:100px" data-col="customer">Customer</th>
                            <th class="<?= t('th') ?>" style="flex:12 1 0;min-width:80px" data-col="quotation_date">Quotation Date</th>
...
<div id="colPanel"
    ...
    <div class="flex-1 overflow-y-auto py-2 px-5">
        ...
        <div class="flex items-center gap-3 py-2.5 select-none">
            ...
            <span class="text-sm text-slate-400">Quotation #</span>
        ...
        <label class="flex items-center gap-3 py-2.5 cursor-pointer rounded-lg hover:bg-slate-50 -mx-1 px-1 transition-colors" onclick="colToggleClick('quotation_date');return false;">
            <span id="colCb_quotation_date" class="col-cb inline-flex w-4 h-4 rounded border-2 border-slate-300 bg-white items-center justify-center shrink-0 transition-colors" data-col="quotation_date"></span>
            <input type="checkbox" class="col-toggle sr-only" data-col="quotation_date">
            <span class="text-sm text-slate-700">Quotation Date</span>
        </label>
...
function invRender() {
    ...
    if (INV_TOTAL === 0) {
        tbody.innerHTML =
            ...
            '</div><p class="text-sm text-slate-400 mb-1">No quotations found.</p>' +
            '<a href="quotation.php?action=new" class="text-sm text-indigo-600 hover:underline">Create your first invoice \u2192</a>' +
            '</td></tr>';
        invRenderPager(0, 0);
    ...
    slice.forEach(function(inv) {
        var invNoSafe = invEsc(inv.quotation_no).replace(/'/g,"\'").replace(/"/g,'&quot;');
        ...
        dataRows.push(
            ...
            ' data-row="' + inv.id + '" data-href="view_quotation.php?id=' + inv.id + '">' +
            '<td class="' + td + ' whitespace-nowrap overflow-hidden" style="flex:14 1 0;min-width:90px" data-col="quotation_no">' +
                '<a href="view_quotation.php?id=' + inv.id + '" class="font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">' + invEsc(inv.quotation_no) + '</a>' +
            '</td>' +
            ...
            '<td class="' + td + ' text-slate-500 whitespace-nowrap overflow-hidden" style="flex:12 1 0;min-width:80px" data-col="quotation_date">' + invEsc(inv.quotation_date) + '</td>' +
        ...
        actionRows.push(
            ...
            '<td class="flex items-center justify-center gap-1 py-2.5 px-1 w-full">' +
                '<a href="view_quotation.php?id=' + inv.id + '" title="View" class="w-7 h-7 flex items-center justify-center rounded-md text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors">' +
                ...
                '</a>' +
                '<a href="quotation.php?action=edit&id=' + inv.id + '" title="Edit" class="w-7 h-7 flex items-center justify-center rounded-md text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-colors">' +
...
document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
    ...
    fetch('delete_quotation.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'id=' + _deleteId
    }).then(r => r.json()).then(d => {
        closeDelete(); btn.disabled = false; btn.textContent = 'Delete';
        if (d.success) { showToast('Quotation deleted.', 'success'); setTimeout(() => location.reload(), 700); }
        else showToast(d.message || 'Failed.', 'error');
    }).catch(() => { closeDelete(); btn.disabled = false; btn.textContent = 'Delete'; showToast('Server error.', 'error'); });
});

// ── Column visibility ────────────────────────────
var COL_KEY      = 'quotation_col_prefs_v2';
var COL_DEFAULTS = {customer:true, quotation_date:true, due_date:true, amount:true, status:true};
...
if ($action === 'edit' && !empty($_GET['id'])) {
    ...
    if ($inv) {
        $editMode = true;
        $iStmt    = $pdo->prepare("SELECT * FROM quotation_items WHERE quotation_id = ? ORDER BY sort_order, id");
        $iStmt->execute([$id]);
...
// ── Quotation number formats ────────────────────
$quotationFormats = $pdo->query("SELECT * FROM number_formats WHERE doc_type='quotation' ORDER BY is_default DESC, id")->fetchAll();
$nextNo = '';
$defaultFormatId = 0;
if (!$editMode && !empty($quotationFormats)) {
    $defaultFormatId = $quotationFormats[0]['id'];
    $nextNo = computeNextNo($pdo, $quotationFormats[0]['format']);
}
...
try {
    $products_for_quotation = $pdo->query("
        ...
        ORDER BY p.name
    ")->fetchAll();
} catch (Exception $e) { $products_for_quotation = []; }
...
$pageTitle = $editMode ? 'Edit Quotation' : 'New Quotation';
$pageSub   = $editMode ? e($inv['quotation_no']) : 'Fill in the details below';
...
<script>
document.getElementById('pageActions').innerHTML = `
    <a href="quotation.php" class="<?= t('btn_base') ?> <?= t('btn_ghost') ?> h-9">Cancel</a>
...
function getPaymentTerms() {
    var list = INVOICE_PT[_quotationPaymentMode] || [];
...
var _quotationPaymentMode = '<?= e($editMode && $inv ? ($inv['payment_mode'] ?? 'cash') : 'cash') ?>';
var _customerPaymentTermId   = null;
var _customerPaymentTermName = '';
</script>

<form id="quotationForm" method="POST" action="save_quotation.php" enctype="multipart/form-data" novalidate>
<?php if ($editMode): ?>
<input type="hidden" name="edit_id"    value="<?= $inv['id'] ?>">
<?php endif; ?>
<input type="hidden" name="status"               id="formStatus"       value="<?= e($editMode ? $inv['status'] : 'draft') ?>">
<input type="hidden" name="quotation_format_id"  id="quotationFormatId" value="<?= e($defaultFormatId) ?>">
...
<div class="grid grid-cols-[200px_1fr]">
    <div class="p-6 border-r border-slate-100">
        ...
        <p class="text-xs text-slate-400 leading-relaxed">Quotation number, date and general information.</p>
    </div>
    <div class="p-6">
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="<?= t('label') ?>">Quotation Number <span class="text-red-400">*</span></label>
                <?php if ($editMode): ?>
                    <!-- Locked in edit mode -->
                    <div class="<?= t('input') ?> font-mono text-slate-700 bg-slate-50 cursor-not-allowed flex items-center justify-between">
                        <span><?= e($inv['quotation_no']) ?></span>
                    ...
                    <input type="hidden" name="quotation_no" value="<?= e($inv['quotation_no']) ?>">
                <?php elseif (empty($quotationFormats)): ?>
                ...
                <?php else: ?>
                    <div id="quotationNoDd" class="relative">
                        <!-- Trigger -->
                        <button type="button" id="quotationNoBtn" onclick="quotationNoDdToggle()"
                                style="outline:none"
                                class="<?= t('input') ?> text-left flex items-center justify-between font-mono">
                            <span id="quotationNoDisplay"><?= e($nextNo) ?></span>
                            <svg id="quotationNoChevron" class="w-4 h-4 text-slate-400 shrink-0 transition-transform"
                        ...
                        <!-- Dropdown panel -->
                        <div id="quotationNoDdPanel" style="display:none"
                             class="absolute z-50 left-0 top-full mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden">
                            <ul class="py-1">
                                <?php foreach ($quotationFormats as $f): ?>
                                <li>
                                    <button type="button"
                                            onclick="quotationNoDdSelect(<?= $f['id'] ?>, '<?= e(addslashes($f['format'])) ?>')"
                                            id="quotationNoOpt_<?= $f['id'] ?>"
                        ...
                        <input type="hidden" name="quotation_no" id="quotationNoValue" value="<?= e($nextNo) ?>">
                    </div>
                <?php endif; ?>
            </div>
            <div>
            ...
            <div>
                ...
                <div class="relative dp-wrap">
                    <input type="text" id="quotationDate" name="quotation_date" required readonly
                           value="<?= e($editMode ? date('d/m/Y', strtotime($inv['quotation_date'])) : date('d/m/Y')) ?>"
                           placeholder="DD/MM/YYYY"
                           class="<?= t('input') ?> cursor-pointer pr-8">
                    <svg class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>
                </div>
            ...
            <div>
                ...
                <div id="quotationCurrencyDd" x-data="quotationCurrencyComp('<?= e($editMode ? $inv['currency'] : 'MYR') ?>')" class="relative">
                    <!-- Trigger -->
                    <div class="relative">
                        <input type="text" id="quotationCurrencyInput"
            ...
            <div>
                ...
                <input type="hidden" name="payment_mode" id="quotationPaymentMode" value="<?= e($curPayMode) ?>">
                <input type="hidden" name="payment_term_id" id="quotationPtHiddenInForm" value="<?= e($editMode && $inv ? ($inv['payment_term_id'] ?? '') : '') ?>">
            </div>
...
?>
                ...
                <tr class="item-desc-row border-b border-slate-50 transition-colors">
                    ...
                    <td class="px-3 pb-2 pt-1" colspan="1">
                    ...
                    <td class="px-2 pb-2 pt-1" colspan="5">
                        <input type="hidden" name="items[<?= $i ?>][classification]" class="item-class" value="">
                    </td>
...
if ($editMode) {
    try {
        $pStmt = $pdo->prepare("SELECT * FROM quotation_payments WHERE invoice_id=? ORDER BY id");
        $pStmt->execute([$id]);
    ...
    <div class="p-6">
        ...
        <div id="fileList" class="mt-3 space-y-2">
            <?php foreach ($existingAttachments as $att):
                $attPath = APP_ROOT . '/storage/attachments/' . $att['stored_name'];
                $attUrl  = str_replace(rtrim($_SERVER['DOCUMENT_ROOT'], '/'), '', APP_ROOT) . '/storage/attachments/' . rawurlencode($att['stored_name']);
...
const PRODUCTS  = <?= json_encode(array_values($products_for_quotation), JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT) ?>;
...
(function() {
    ...
    var input   = document.getElementById('quotationDate');
...
function submitForm(fallback) {
    ...
    var fp = document.getElementById('quotationDate');
    if (fp && fp.dataset.isoVal) fp.value = fp.dataset.isoVal;

    // Quotations do not check stock — save directly
    var form = document.getElementById('quotationForm');
    doSaveQuotation(form);
}

function doSaveQuotation(form) {
    ...
    .then(function(d) {
        if (d.success) {
            ...
            setTimeout(function() {
                window.location.href = 'view_quotation.php?id=' + d.id;
            }, 600);
        } else {
...
function customerSearch() {
    return {
        ...
        onKey: function(e) {
            ...
            if (e.key === 'Enter') {
                ...
                if (list.length > 0 && list[idx]) {
                    ...
                    var next = document.querySelector('input[name="quotation_no"]');
                    if (next) next.focus();
                }
                return;
            }
        ...
        select: function(c) {
            ...
            if (c.currency) {
                var currEl = document.getElementById('quotationCurrencyDd');
            ...
            var ptEl = document.getElementById('quotationPtDd');
        ...
        clearCustomer: function() {
            ...
            var ptEl2 = document.getElementById('quotationPtDd');
...
function markAttachmentDeleted(btn, attId, attName) {
    ...
    inp.value = attId;
    (document.getElementById('quotationForm') || document.querySelector('form')).appendChild(inp);
}
...
function quotationCurrencyComp(initialCode) {
    ...
    return {
        ...
        onFocus: function() {
            ...
            var self = this;
            this.$nextTick(function() {
                var el = document.getElementById('quotationCurrencyInput');
                if (el) el.select();
            });
        },
        pick: function(c) {
            ...
            this.activeIdx = -1;
            // Blur so next click fires @focus fresh
            var inp = document.getElementById('quotationCurrencyInput');
            if (inp) inp.blur();
        },
        pickActive: function() {
...
function quotationPtComp() {
    return {
        open:        false,
        selectedId:  '',
        selectedName:'',
        get list() { return INVOICE_PT[_quotationPaymentMode] || []; },
        init() {
            ...
            if (found) {
                ...
                var f = document.getElementById('quotationPtHiddenInForm');
...
var _quotationNoDdOpen = false;
var _quotationNoDdActiveId = <?= $defaultFormatId ?: 0 ?>;

function quotationNoDdToggle() {
    var panel   = document.getElementById('quotationNoDdPanel');
    var chevron = document.getElementById('quotationNoChevron');
    if (!panel) return;
    _quotationNoDdOpen = !_quotationNoDdOpen;
    panel.style.display = _quotationNoDdOpen ? 'block' : 'none';
    if (chevron) chevron.style.transform = _quotationNoDdOpen ? 'rotate(180deg)' : '';
}

function quotationNoDdSelect(formatId, formatStr) {
    _quotationNoDdActiveId = formatId;
    // Highlight selected option
    document.querySelectorAll('[id^="quotationNoOpt_"]').forEach(function(btn) {
        btn.className = 'w-full text-left px-4 py-2.5 text-sm font-mono transition-colors text-slate-800 hover:bg-slate-50';
    });
    var active = document.getElementById('quotationNoOpt_' + formatId);
    ...
    var display = document.getElementById('quotationNoDisplay');
    var hidden  = document.getElementById('quotationNoValue');
    var fmtId   = document.getElementById('quotationFormatId');
...
document.addEventListener('click', function(e) {
    if (!_quotationNoDdOpen) return;
    var dd = document.getElementById('quotationNoDd');
    if (dd && !dd.contains(e.target)) {
        _quotationNoDdOpen = false;
        var panel   = document.getElementById('quotationNoDdPanel');
        var chevron = document.getElementById('quotationNoChevron');
        if (panel)   panel.style.display = 'none';
...
// ── end quotation number dropdown ──────────────────────────────────

// ── Payment Mode toggle ───────────────────────────────────────────
function setPaymentMode(mode) {
    document.getElementById('quotationPaymentMode').value = mode;
        ...
        <div class="px-6 pt-5 pb-4 border-b border-slate-100">
            ...
            <div class="grid grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div>
                    ...
                    <div id="quotationPtDd" x-data="quotationPtComp()">
                        <div class="relative">
                            <button type="button"
                                @click="open=!open"
                        ...
                        <input type="hidden" name="payment_term_id" id="quotationPtHidden" :value="selectedId">
                    </div>
                </div>
